...
from tools.route_generation import selectRoad as ST
def run_duarouter(net_file, initial_trip_file, final_output_alt, final_output_rou):
    ...
    for time in range(5):
        ...
        if os.path.exists(temp_output_alt):
            ...
            break
    if os.path.exists("./data/legacy/trips.xml"):
        os.remove("./data/legacy/trips.xml")
...
def generate_trip(road_info):  
    ...
    for index,roadName in  enumerate(road_info):
        ET.SubElement(trip,
                      ...
                      length = "5",
                      maxSpeed = str(max(1.0, (float(road_info[roadName]["AvgSpd"]) / percentage) /3.6)))
    ...
    ET.ElementTree(trip).write("./data/legacy/trips.xml",encoding="utf-8",xml_declaration=True)
    print("已生成")

if __name__ == "__main__":
    generate_trip(ST.select())
    run_duarouter("./data/legacy/ntut_network_split.net.xml", "./data/legacy/trips.xml", "./data/legacy/output.rou.alt.xml","./data/legacy/output.rou.xml")